...
rlSocket       sock("127.0.0.1",9502,1);
rlSharedMemory shm("/home/jjmg/automation/shm/modbus.shm",9);
rlMailbox      mbx("/home/jjmg/automation/mbx/modbus.mbx");
rlThread       thread;
rlThread       watchdog;
short          lifeCounter     = 0;
...
int main(int ac, char **av)
{
  ...
  while(1)
  {
    ...
    ret = modbusCycle(offset,1,3,3,1);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,1,1,0,5);
    if(ret>0) offset += ret; else continue;
  }
